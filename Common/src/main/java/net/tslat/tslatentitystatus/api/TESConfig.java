...
import net.tslat.tslatentitystatus.api.client.constant.TESHUDActivation;
...
import net.tslat.tslatentitystatus.core.client.hud.TESHudPosition;
import org.apache.commons.lang3.tuple.Pair;

/// Client configuration interface for `TES`' configurable settings
public final class TESConfig {
	...
	TESConfig(final ModConfigSpec.Builder config) {
		...
		this.inWorldHudManualVerticalOffset = config
				...
				.defineInRange("inWorldHudManualVerticalOffset", 0, -Double.MAX_VALUE, Double.MAX_VALUE);
	...
	/// Whether the [TESHud] should be rendered or not
	public boolean hudEnabled() {
		return this.hudEnabled.get();
	}

	/// What position the [TESHud] should render in
	public TESHudPosition hudRenderPosition() {
		return this.hudRenderPosition.get();
	}

	/// Manually adjust the left-offset rendering position of the [TESHud]
	public int hudPositionLeftAdjust() {
		return this.hudPositionLeftAdjust.get();
	}

	/// Manually adjust the top-offset rendering position of the [TESHud]
	...
	/// Whether the [TESHud] should render the entity's image
	public boolean hudEntityRender() {
		return this.hudEntityRender.get();
	}

	/// Whether the [TESHud] should cull any overflow for entities that don't scale properly to their rendering frame
	public boolean hudPreventEntityOverflow() {
		return this.hudPreventEntityOverflow.get();
	}

	/// Whether the entity rendered in the [TESHud] should keep the red overlay when the real entity takes damage
	public boolean hudEntityDamageOverlay() {
		return this.hudEntityDamageTint.get();
	}

	/// Whether the [TESHud] Should render the entity's name
	...
	/// Whether the [TESHud] Should render for boss-type entities
	public boolean hudBossesEnabled() {
		return this.hudBossesEnabled.get();
	}

	/// Whether the [TESHud] should render the entity's stats
	...
	/// Whether the [TESHud] should render the entity's alignment icons
	public boolean hudEntityIcons() {
		return hudEntityIcons.get();
	}

	/// Whether the [TESHud] should render the entity's potion icons
	public boolean hudPotionIcons() {
		return hudPotionIcons.get();
	}

	/// Whether the [TESHud] should render horses' stats
	public boolean hudHorseStats() {
		return hudHorseStats.get();
	}

	/// Get the rendered opacity of the [TESHud]
	///
	/// This affects the entire HUD
	///
	...
	/// Get the health render type for the [TESHud]
	///
	/// Available options are:
	///
	///   - NUMERIC - Use numeric values for health only
	///   - BAR - Use a health-bar style render
	///   - BAR\_ICONS - Display in the BAR style, but with the health numerically displayed in the stats icons row
	///   - COMBINED - Use a health-bar style render with numeric values overlaid
	...
	/// Whether the health bar in the [TESHud] should render with health bar segments
	public boolean hudHealthBarSegments() {
		return this.hudHealthBarSegments.get();
	}

	/// How long the [TESHud] health bar should be (in pixels)
	...
	/// Whether the [TESHud] should be rendered for the player or not
	...
	/// Get the bar render type for the in-world `TES` entity status HUD
	///
	/// Available options are:
	///
	///   - NUMERIC - Use numeric values only
	...
	/// What style [TESParticle]s' font should render in
	...
	/// How many decimals the numeric [TESParticle]s should round to
	public int particleDecimalPoints() {
		return this.particleDecimalPoints.get();
	}

	/// Get the scale modifier for [TESParticle] rendering
	///
	...
	/// Get the **ARGB** packed-int colour value of the damage-type [TESParticle]
	public int getDamageParticleColour() {
		return this.damageParticleColour.get();
	}

	/// Get the **ARGB** packed-int colour value of the healing-type [TESParticle]
