...
public class TextRenderHelper {
    ...
    public void inWorldSubmit(TESHudRenderContext.InWorldArgs args, float x, float y) {
        ...
        args.renderTasks().submitCustomGeometry(args.poseStack(), RenderTypes.textBackgroundSeeThrough(), (pose, _) -> {
    ...
    public enum Style {
        ...
        interface RenderFunction {
            List<Pair<Font.PreparedText, @Nullable ScreenRectangle>> prepare(Font font, FormattedCharSequence charSequence, float x, float y,
                                                                             ...
                                                                             Either<Matrix3x2f, Matrix4f> pose, @Nullable ScreenRectangle scissor, MultiBufferSource.@Nullable BufferSource bufferSource);
        }
        ...
        private static List<Pair<Font.PreparedText, @Nullable ScreenRectangle>> renderDefault(Font font, FormattedCharSequence charSequence, float x, float y,
            ...
            return Either.unwrap(pose.mapBoth(pose2d -> {
                ...
                if (region != null) {
                    ...
                    if (scissor != null)
                        region = scissor.intersection(region);
                }

                return List.of(Pair.of(preparedText, region));
            }, pose3d -> {
