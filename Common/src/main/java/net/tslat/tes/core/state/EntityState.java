		...
		if (TESAPI.getConfig().particlesEnabled()) {
			...
			if (healthDelta < 0) {
				...
				if (TESAPI.getConfig().verbalHealthParticles() && this.currentHealth <= 0 && this.lastHealth >= this.entity.getMaxHealth()) {
					particle = new ComponentParticle(this, particlePos, TESParticle.Animation.POP_OFF, new TranslatableComponent("config.tes.particle.verbal.instakill").setStyle(Style.EMPTY.withColor(TESAPI.getConfig().getDamageParticleColour())));
				}
				else {
				...
				if (TESAPI.getConfig().verbalHealthParticles() && this.currentHealth >= this.entity.getMaxHealth() && this.lastHealth <= this.entity.getMaxHealth() * 0.05f) {
					particle = new ComponentParticle(this, particlePos, TESParticle.Animation.RISE, new TranslatableComponent("config.tes.particle.verbal.fullHeal").setStyle(Style.EMPTY.withColor(TESAPI.getConfig().getHealParticleColour())));
				}
				else {
