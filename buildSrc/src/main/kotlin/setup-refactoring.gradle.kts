...
tasks.register("refactorOnInitialSetup", Action<Task> {
    ...
    try {
        for (module in arrayOf("common", "fabric", "forge")) {
...
private fun isInitialUnmodifiedSetup(): Boolean {
    ...
    if (commonProject != null) {
        for (path in arrayOf(
            "src/main/resources/assets/mymod", "src/main/resources/data/mymod", "src/main/resources/mymod.mixins.json")) {
            if (!commonProject.projectDir.resolve(path).exists())
                return false
        }

        if (!commonProject.projectDir.resolve("src/main/resources/mymod.accesswidener").exists())
            return false
    }
