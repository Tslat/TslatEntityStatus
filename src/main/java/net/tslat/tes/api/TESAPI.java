...
public final class TESAPI {
	...
	public static void addTESParticle(LivingEntity targetedEntity, ITextComponent contents) {
		if (TESConstants.IS_SERVER_SIDE) {
			...
			if (entityState != null)
				TESParticleManager.addParticle(new ComponentParticle(entityState, new Vector3f(targetedEntity.getEyePosition(1)), contents));
		}
	}
	...
	public static void sendParticle(LivingEntity targetedEntity, double value, int colour) {
		if (TESConstants.IS_SERVER_SIDE) {
			...
			if (entityState != null)
				TESParticleManager.addParticle(new NumericParticle(entityState, new Vector3f(targetedEntity.getEyePosition(1)), value).withColour(colour));
		}
	}
}
